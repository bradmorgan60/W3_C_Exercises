#include <stdio.h>

/*
Write a C program to display multiple variables.
Sample Variables :
a+ c, x + c,dx + x, ((int) dx) + ax, a + x, s + b, ax + b, s + c, ax + c, ax + ux
Declaration :
int a = 125, b = 12345;
long ax = 1234567890;
short s = 4043;
float x = 2.13459;
double dx = 1.1415927;
char c = 'W';
unsigned long ux = 2541567890;
*/

int main() 
{
    int a = 125, b = 12345;
    long ax = 1234567890;
    short s = 4043;
    float x = 2.13459;
    double dx = 1.1415927;
    char c = 'W';
    unsigned long ux = 2541567890;

    printf("a + c = %d \n", a + c);
    printf("x + c = %f \n", x + c);
    printf("dx + x = %f \n", dx + x);
    printf("((int) dx) + ax = %ld \n", ((int) dx + ax));
    printf("a + x = %lf \n", a + x);
    printf("s + b = %d \n", s + b);
    printf("ax + b = %ld \n", ax + b);
    printf("s + c = %d \n", s + c);
    printf("ax + c = %ld \n", ax + c);
    printf("ax + ux = %lu \n", ax + ux);

    return 0;
}

/*
Placeholders for printf:
%d: Represents a signed integer.
%u: Represents an unsigned integer.
%ld: Represents a long signed integer.
%lu: Represents a long unsigned integer.
%lld: Represents a long long signed integer.
%llu: Represents a long long unsigned integer.
%f: Represents a floating-point number in decimal notation.
%lf: Represents a double-precision floating-point number in decimal notation.
%e or %E: Represents a floating-point number in scientific notation.
%c: Represents a character.
%s: Represents a string.
%%: Represents a literal % character.
*/